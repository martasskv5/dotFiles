...
# Monitor details (fallback defaults for non-Hypr compositors)
scale_factor="1"
monitor_height="1080"

if command -v hyprctl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1 && [[ -n "$focused_monitor" ]]; then
...
modify_startup_config() {
  ...
  local state_file="$HOME/.config/niri/wallpaper_effects/.wallpaper_startup_last"
